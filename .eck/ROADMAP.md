...
- [x] **Track estimation — least-action smoothing + road-constrained matching (SPEC'd + BUILT 2026-07-06, owner physics framing)** — see `.eck/TRACK_ESTIMATION.md` (status header = what shipped; live E2E: Karlsruhe→Speyer 57.3 km smoothed → 56.9 km road, share 1.00, 888 ms). Two tiers, all on-device: streaming forward-Kalman (O(1)/point → live odometer/Expense prefill/plausibility band/live marker) + final RTS smoother with innovation gating on triggers (trip end / expense receipt needs mileage / stop without odometer photo). Phase 2: Viterbi road snap over corridor MVT tiles from the existing MapLibre offline cache → `matched_km`. Prototype on the 2026-07-06 tracks: 1072 km raw chaos → 154.4 km (road ≈150), 4 junk towers (all OpenCelliD samples=1, worst → Kassel 163 km off) auto-rejected by the gate, ~10 ms/trip in Kotlin. NO neural net, NO country-wide graph — rejected as overkill.
